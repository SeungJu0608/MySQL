...
-- CROSS JOIN(Cartesian Product)은 join조건을 기술하지 않은 경우 발
...
SELECT * FROM EMP
;
SELECT  STAFF.*, MANAGER.EMPNO AS mangr, MANAGER.ENAME AS MANGR_NAME
FROM    EMP AS STAFF,
        EMP AS MANAGER
...
SELECT  *
FROM    Customer AS C
FULL OUTER JOIN Orders                  # MySQL은 FULL JOIN 문법이 없어.. 대신 UNION을 이용!!
...
(=대한민국에 거주하는 고객의 이름과 도서를 주문한 고객의 이름을 보이시오)      # OR조건!!
...
UNION                           # 컬럼이 동일해야 합칠 수 있다!! 즉, 조회하는 열이 같고 차원이 같아야한다. LIKE 행렬!!
